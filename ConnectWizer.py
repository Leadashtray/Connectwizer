...
class ConnectWizer():
    ...
    def make_get_request(self, slug):
        ...
        '''
        r = requests.get(cwUrl + slug, headers=self.get_headers())
    ...
    def make_post_request(self, slug, context):
        ...
        r = requests.post(cwUrl + slug, headers=self.get_headers(), data=context)
        return self.make_request(r)
    ...
    def make_put_request(self, slug, context):
        ...
        '''
        context = json.dumps(context)
        r = requests.put(cwUrl + slug, headers=self.get_headers(), data=context)
        return self.make_request(r)
    ...
    def make_patch_request(self, slug, context):
        ...
        '''
        context = json.dumps(context)
        r = requests.patch(cwUrl + slug, headers=self.get_headers(), data=context)
        return self.make_request(r)
    ...
    def make_delete_request(self, slug):
        ...
        :return: NONE
        '''
        r = requests.delete(cwUrl + slug, headers=self.get_headers())
        try:
            # request has been made
            r.raise_for_status()
        except:
